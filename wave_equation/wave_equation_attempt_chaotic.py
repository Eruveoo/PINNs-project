...
def source_term(x):
    ...
    x_ = x[:, 1:2]
    y_ = x[:, 2:3]
    return 0.1 * torch.sin(2*np.pi*t) * torch.exp(-5*(x_**2 + y_**2))
...
def initial_u(xy):
    ...
    y = xy[:, 1:2]
    return (
        torch.sin(np.pi * x) * torch.sin(np.pi * y)
        + 0.2 * torch.sin(5 * np.pi * x) * torch.sin(3 * np.pi * y)
    )
...
def train_wave_model():
    ...
    for epoch in range(epochs):
        X_interior, X_boundary, X_initial = generate_wave_points(5000, 500, 500)
